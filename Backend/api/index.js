...
// Updated: 2025-09-15 - Fixed deployment conflicts
...
const allowedOrigins = [
  ...
  // Mobile app support - Expo development
  'exp://10.75.145.132:8081', // Expo development server
  'exp://localhost:8081', // Local Expo server
  'exp://192.168.1.100:8081', // Local network Expo server
  'exp://192.168.0.100:8081', // Alternative local network
  'exp://192.168.1.101:8081', // Additional local network
  'exp://192.168.0.101:8081', // Additional local network
  ...
  // Production mobile app domains (if any)
...
app.use(cors({
  origin: (origin, callback) => {
    ...
    if (origin.match(/^exp:\/\/.*$/)) {
      console.log('✅ CORS allowing Expo URL:', origin);
      return callback(null, true);
    }
...
app.get('/', (req, res) => {
  res.json({
    message: 'Mathematico API Server - DEPLOYMENT FIXED',
    version: '1.3.0',
    status: 'running',
    ...
    fix: 'deployment-conflict-resolved',
    endpoints: {
...
// Health check endpoint - Enhanced for mobile connectivity
...
// Additional health endpoint for root path
app.get('/health', (req, res) => {
  res.json({
    ok: true,
    status: 'healthy',
    timestamp: new Date().toISOString(),
    message: 'Mathematico API is running'
  });
});
